...
vi.mock("@/lib/hooks/use-request-query", () => ({
  useRequestQuery: (): void => {},
}));
...
vi.mock("@/components/onboarding/code-panel", () => ({
...
vi.mock("@/lib/utils/handle-send", () => ({ handleSend: h.handleSendSpy }));
...
describe("RestClient", () => {
  it("sends request with variable substitution and displays the response", async () => {
    renderClient();
  ...
  it("Prettify formats valid JSON and does not show an error", async () => {
    renderClient();
  ...
  it("Prettify shows toast error on invalid JSON", async () => {
  ...
  it("Send button is disabled when URL is empty or whitespace", () => {
    h.state.url = `   `;
    renderClient();
    const sendBtn = screen.getByRole("button", { name: "Send" });
  ...
  it("Add/Delete header buttons trigger correct store actions", () => {
    renderClient();
  ...
  it("Tabs are clickable and placeholders are present", () => {
    renderClient();
  ...
  it("Changing HTTP method calls setMethod", () => {
    renderClient();
  ...
  it("Typing in URL input calls setUrl", () => {
    renderClient();
